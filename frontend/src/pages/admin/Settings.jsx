...
const CouriersTab = () => {
  ...
  const assign = async (orderId) => {
    if (!selected) return toast.error('Válassz futárt');
    await updateOrder(orderId, { courierId: selected, status: 'on_route' });
      ...
      <div className="col-span-7">
        <div className="bg-white rounded-xl border border-neutral-200 p-5">
          ...
          <div className="space-y-2">
            {pending.map((o) => (
              <div key={o.id} className="flex items-center gap-3 p-3 rounded-lg border border-neutral-200">
                ...
                <button onClick={() => assign(o.id)} disabled={!selected} className={`inline-flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-md border ${selected ? 'bg-neutral-900 text-white border-neutral-900 hover:bg-neutral-800' : 'bg-neutral-100 text-neutral-400 border-neutral-200 cursor-not-allowed'}`}>Odaadom <ArrowRight size={12} /></button>
              </div>
            ))}
          ...
          <div className="space-y-2">
            {couriers.map((c) => (
              <div key={c.id} className={`flex items-center gap-3 p-3 rounded-lg border ${selected === c.id ? 'border-neutral-900 bg-neutral-50' : 'border-neutral-200'}`}>
                ...
                <button onClick={async () => { await deleteCourier(c.id); if (selected === c.id) setSelected(null); toast.success('Futár törölve'); }} className="h-8 w-8 rounded-md border border-neutral-200 text-rose-500 flex items-center justify-center hover:bg-rose-50"><Trash2 size={14} /></button>
              </div>
            ))}
          </div>
        </div>
